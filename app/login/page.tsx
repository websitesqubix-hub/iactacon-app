...
export default function Login() {
  ...
  return (
    <div style={{ background:"#fdf6ec", minHeight:"100vh", fontFamily:"system-ui,-apple-system,sans-serif" }}>
      ...
      {/* ===== TOP: LOGO + CONFERENCE DETAILS ===== */}
      <div style={{ background:"linear-gradient(150deg,#1a3a5c,#3d6b82)", padding:"28px 20px 32px", position:"relative", overflow:"hidden", textAlign:"center" }}>
        {/* Ornament rings */}
        <div style={{ position:"absolute", top:-30, right:-30, width:130, height:130, borderRadius:"50%", border:"1px solid rgba(201,168,76,0.25)", pointerEvents:"none" }}/>
        <div style={{ position:"absolute", top:-10, right:-10, width:86, height:86, borderRadius:"50%", border:"1px solid rgba(201,168,76,0.12)", pointerEvents:"none" }}/>
        <div style={{ position:"absolute", bottom:-30, left:-30, width:100, height:100, borderRadius:"50%", border:"1px solid rgba(255,255,255,0.06)", pointerEvents:"none" }}/>

        {/* Logo */}
        <div style={{ position:"relative", width:86, height:86, margin:"0 auto 14px", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ position:"absolute", width:86, height:86, borderRadius:"50%", border:"1px solid rgba(201,168,76,0.35)", animation:"spin 20s linear infinite" }}/>
          <div style={{ position:"absolute", width:70, height:70, borderRadius:"50%", border:"1px dashed rgba(201,168,76,0.2)", animation:"spin-rev 15s linear infinite" }}/>
          <svg style={{ position:"absolute", width:86, height:86 }} viewBox="0 0 86 86">
            <circle cx="43" cy="1.5" r="2.2" fill="#c9a84c" opacity="0.9"/>
            <circle cx="84.5" cy="43" r="2.2" fill="#c9a84c" opacity="0.9"/>
            <circle cx="43" cy="84.5" r="2.2" fill="#c9a84c" opacity="0.9"/>
            <circle cx="1.5" cy="43" r="2.2" fill="#c9a84c" opacity="0.9"/>
          </svg>
          <div style={{ width:60, height:60, borderRadius:"50%", border:"2px solid rgba(201,168,76,0.5)", overflow:"hidden", background:"rgba(255,255,255,0.1)", zIndex:2 }}>
            <img src="https://iactacon2027.com/wp-content/uploads/2026/05/iacta-site-logo.jpeg" style={{ width:"100%", height:"100%", objectFit:"cover", borderRadius:"50%" }}/>
          </div>
        </div>

        <div style={{ fontSize:9, color:"rgba(255,255,255,0.55)", letterSpacing:3, textTransform:"uppercase" as const, marginBottom:4 }}>Official Conference App</div>
        <div style={{ fontSize:26, fontWeight:800, color:"white", lineHeight:1 }}>IACTACON</div>
        <div style={{ fontSize:26, fontWeight:800, background:"linear-gradient(135deg,#c9a84c,#f0d060)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", lineHeight:1, marginBottom:8 }}>2027</div>
        <div style={{ fontSize:11, color:"rgba(255,255,255,0.6)", maxWidth:240, margin:"0 auto", lineHeight:1.5, marginBottom:14 }}>
        ...
        <div style={{ display:"flex", gap:6, justifyContent:"center", marginBottom:16 }}>
          {["Safety","Science","Skill"].map(t => (
            <div key={t} style={{ background:"rgba(255,255,255,0.1)", border:"1px solid rgba(201,168,76,0.3)", borderRadius:20, padding:"3px 12px", color:"rgba(255,255,255,0.85)", fontSize:9, fontWeight:600 }}>{t}</div>
          ))}
        </div>

        {/* Dates bar */}
        <div style={{ display:"flex", background:"rgba(0,0,0,0.2)", borderRadius:12, overflow:"hidden", border:"1px solid rgba(201,168,76,0.2)" }}>
          {[{l:"Workshop",v:"18 Feb 2027"},{l:"Conference",v:"19–21 Feb"},{l:"Venue",v:"Ozone, Kolkata"}].map((d,i) => (
            <div key={i} style={{ flex:1, textAlign:"center", padding:"8px 0", borderRight:i<2?"1px solid rgba(255,255,255,0.08)":"none" }}>
      ...
      <div style={{ padding:"20px 16px 0" }}>
        ...
        {/* Login Card */}
        <div style={{ background:"white", borderRadius:18, padding:"0 16px 16px", border:"1px solid rgba(201,168,76,0.2)", boxShadow:"0 4px 24px rgba(91,143,168,0.12)", overflow:"hidden", marginBottom:14 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
            <div onClick={() => setRemember(!remember)} style={{ width:16, height:16, borderRadius:4, border:`1.5px solid ${remember?"#5b8fa8":"rgba(91,143,168,0.35)"}`, background:remember?"rgba(91,143,168,0.12)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0 }}>
            ...
            <span style={{ fontSize:11, color:"#5b8fa8", cursor:"pointer" }} onClick={() => setRemember(!remember)}>Keep me signed in</span>
          </div>
          ...
          <button onClick={handleLogin} disabled={loading} style={{ width:"100%", padding:13, background:loading?"rgba(26,58,92,0.5)":"linear-gradient(135deg,#1a3a5c,#3d6b82)", border:"none", borderRadius:12, color:"white", fontSize:14, fontWeight:700, cursor:loading?"not-allowed":"pointer", position:"relative", overflow:"hidden" as const }}>
        ...
        <div style={{ background:"white", borderRadius:14, border:"1px solid rgba(201,168,76,0.2)", overflow:"hidden", marginBottom:10 }}>
          <div style={{ height:2, background:"linear-gradient(90deg,#c9a84c,#f0d060,#c9a84c)" }}/>
          <div style={{ padding:"12px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:10 }} onClick={() => setShowForgot(!showForgot)}>
            <div style={{ width:34, height:34, borderRadius:10, background:"#fff8e1", border:"1px solid rgba(201,168,76,0.3)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#c9a84c" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3M12 17h.01"/></svg>
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:12, fontWeight:700, color:"#1a3a5c" }}>Forgot Registration ID?</div>
              <div style={{ fontSize:10, color:"#7a8a99", marginTop:1 }}>Find using phone & date of birth</div>
            </div>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#c9a84c" strokeWidth="2" style={{ transform:showForgot?"rotate(90deg)":"rotate(0deg)", transition:"0.2s" }}><polyline points="9,18 15,12 9,6"/></svg>
          </div>
          {showForgot && (
        ...
        {/* Not Registered */}
        <a href="https://iactacon2027.com/registration/" target="_blank" style={{ textDecoration:"none", display:"block", marginBottom:20 }}>
          <div style={{ background:"linear-gradient(135deg,#f0f8ff,#e8f4f8)", borderRadius:14, padding:"12px 14px", border:"1px solid rgba(91,143,168,0.2)", display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:34, height:34, borderRadius:10, background:"rgba(91,143,168,0.1)", border:"1px solid rgba(91,143,168,0.2)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5b8fa8" strokeWidth="2" strokeLinecap="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:12, fontWeight:700, color:"#1a3a5c" }}>Not yet Registered?</div>
              <div style={{ fontSize:10, color:"#5b8fa8", fontWeight:600, marginTop:1 }}>Register Now →</div>
            </div>
          </div>
        </a>
      </div>
      ...
      <div style={{ position:"relative", width:"100%", overflow:"hidden" }}>
        {/* Top fade into cream */}
        <div style={{ position:"absolute", top:0, left:0, right:0, height:40, background:"linear-gradient(to bottom,#fdf6ec,transparent)", zIndex:2, pointerEvents:"none" }}/>
        ...
        />
        {/* Bottom overlay with caption + credit */}
        <div style={{ position:"absolute", bottom:0, left:0, right:0, background:"linear-gradient(transparent,rgba(26,58,92,0.85))", padding:"24px 16px 16px", zIndex:2 }}>
          <div style={{ color:"rgba(255,255,255,0.7)", fontSize:10, fontWeight:600 }}>📍 Kolkata — City of Joy · Venue of IACTACON 2027</div>
          <div style={{ marginTop:8, textAlign:"center", fontSize:10, color:"rgba(255,255,255,0.45)", lineHeight:1.6 }}>
            Designed with <span style={{ color:"#f472b6" }}>♥</span> by{" "}
            <span style={{ color:"rgba(201,168,76,0.8)", fontWeight:600 }}>Qubix Infosystems</span>
          </div>
        </div>
      </div>
